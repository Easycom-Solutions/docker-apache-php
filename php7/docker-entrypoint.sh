...
if [[ ! -z $FPM_IDLE_TIMEOUT ]]; then
	sed -i "s,FastCgiExternalServer /usr/lib/cgi-bin/php7.0-fcgi -socket /run/php/php7.0-fpm.sock -pass-header Authorization,FastCgiExternalServer /usr/lib/cgi-bin/php7.0-fcgi -socket /run/php/php7.0-fpm.sock -idle-timeout $FPM_IDLE_TIMEOUT -pass-header Authorization," /etc/apache2/conf-available/php7-fpm.conf
fi

if [[ ! -z $FPM_SLOWLOG_TIMEOUT ]]; then
	sed -i "s,request_slowlog_timeout = 5,request_slowlog_timeout = $FPM_SLOWLOG_TIMEOUT," /etc/php/7.0/fpm/pool.d/www.conf
fi
